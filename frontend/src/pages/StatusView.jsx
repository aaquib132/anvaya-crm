...
export default function StatusView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const statusLeads = leadsRes.data.filter(l => l.status === decodeURIComponent(status));
        setLeads(statusLeads);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const processedLeads = useMemo(() => {
    ...
    if (agentFilter !== "All") {
      result = result.filter(l => (l.salesAgent?._id === agentFilter || l.salesAgent?.id === agentFilter || l.salesAgent === agentFilter));
    }
    ...
    return result;
  }, [leads, agentFilter, priorityFilter, sortBy]);
            ...
            <div className="glass-card p-0 overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60">
               ...
               <div className="px-8 py-6 border-b border-gray-100/60 bg-white/40 flex justify-between items-center backdrop-blur-md">
                 <h2 className="text-xl font-bold text-gray-900 flex items-center gap-3">
                    ...
                    Status: <span className="text-brand-700"><StatusBadge status={decodedStatus} /></span>
                 </h2>
               ...
               <div className="p-6 bg-gray-50/50 border-b border-gray-100/60 flex flex-wrap items-center gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center bg-white px-3 py-1.5 rounded-xl border border-gray-200 shadow-sm focus-within:ring-2 ring-brand-500 transition-shadow">
                      ...
                      <select 
                        className="bg-transparent text-sm font-medium text-gray-700 outline-none w-36 overflow-hidden text-ellipsis"
                        value={agentFilter}
               ...
               <div className="p-0">
                 {processedLeads.length > 0 ? (
                       ...
                       >
                          <div>
                            ...
                            <div className="flex items-center gap-3 mt-2">
                              <span className="text-xs font-semibold text-gray-600 bg-gray-100 px-2 py-1 rounded-md flex items-center gap-1 border border-gray-200">
                   ...
                   <div className="flex flex-col items-center justify-center py-16 text-center px-4">
                     ...
                     <p className="text-sm text-gray-400 max-w-sm">No leads match your current filter settings for this status.</p>
                   </div>
                 )}
               </div>
            </div>
